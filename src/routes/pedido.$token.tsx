...
function Pedido() {
  ...
  return (
    <SiteLayout variant="checkout">
      ...
      <div className="mx-auto w-full max-w-4xl px-3.5 sm:px-6 py-8 sm:py-10 md:py-16">
        {/* BRAND LOGO HEADER */}
        ...
        {/* STATUS BANNER */}
        <div
          className={`rounded-2xl border p-5 sm:p-8 text-center w-full overflow-hidden ${
            paid
        ...
        >
          <div
            className={`mx-auto h-14 w-14 sm:h-16 sm:w-16 rounded-full grid place-items-center ${
          ...
          >
            {paid ? (
              ...
              <Clock className="h-7 w-7 sm:h-8 sm:w-8 text-gold-foreground animate-spin-slow" />
            )}
          </div>
          <h1 className="mt-4 text-xl sm:text-2xl md:text-3xl font-extrabold text-foreground">
        ...
        <div className="mt-6 sm:mt-8 grid gap-4 sm:gap-6 md:grid-cols-2 w-full">
          {/* MAIN PRODUCT CARD */}
          <div className="rounded-2xl border border-border bg-card p-4 sm:p-6 shadow-card flex flex-col justify-between w-full overflow-hidden">
            <div>
              <div className="flex items-center justify-between gap-2 mb-3">
                ...
                {paid && (
                  <span className="rounded-full bg-success/10 text-success text-xs font-bold px-2.5 py-0.5 border border-success/20">
                    Acesso Liberado
                  </span>
                )}
              </div>
              ...
              <div className="flex gap-3 items-center">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 text-base sm:text-lg font-extrabold text-gradient-gold">
              ...
              <div className="mt-6 space-y-2.5">
                {prod?.has_members_area && (
                  <Link to="/membros/$slug" params={{ slug: prod.slug }} className="block">
                    <Button className="w-full gradient-brand text-primary-foreground font-bold text-sm sm:text-base h-11 sm:h-12">
                ...
                {prod?.external_url && (
                  <a href={prod.external_url} target="_blank" rel="noreferrer" className="block">
                    <Button variant="outline" className="w-full text-sm sm:text-base h-11 sm:h-12">
          ...
          {bumpProd && (
            <div className="rounded-2xl border border-gold/40 bg-gold/5 p-4 sm:p-6 shadow-card flex flex-col justify-between w-full overflow-hidden">
              <div>
                <div className="flex items-center justify-between gap-2 mb-3">
                  <span className="text-xs sm:text-sm font-bold uppercase tracking-widest text-gold flex items-center gap-1">
                    <Gift className="h-3.5 w-3.5 shrink-0" /> Oferta Especial Incluída
                  </span>
                  {paid && (
                    <span className="rounded-full bg-success/10 text-success text-xs font-bold px-2.5 py-0.5 border border-success/20">
                      Acesso Liberado
          ...
          <div className="rounded-2xl border border-border bg-card p-4 sm:p-6 shadow-card w-full overflow-hidden">
            <div className="text-xs sm:text-sm font-bold uppercase tracking-widest text-gold mb-4">
              Dados do Pedido
            </div>
            <dl className="space-y-2.5 text-xs sm:text-sm">
              ...
              <Row
                ...
                v={
                  ...
                  >
                    {token.slice(0, 14)}… <Copy className="h-3.5 w-3.5" />
                  </button>
                }
              />
            </dl>
            ...
            {!paid && (
              <div className="mt-4 rounded-xl bg-secondary/70 p-3 text-xs sm:text-sm text-muted-foreground leading-relaxed">
        ...
        {/* FOOTER ACTIONS */}
        <div className="mt-8 sm:mt-10 text-center flex items-center justify-center gap-4">
          <Link to="/loja">
